...
public class ManipulateurCarte {
    ...
    public static Carte chargerCarte(String cheminFichier) {
        return new Carte(Utils.lireContenuFichier(cheminFichier)); // Utilise la méthode utilitaire pour lire le contenu du fichier et créer une carte
    }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(cheminFichier))) {
            ...
            for (int y = 0; y < carte.getHauteur(); y++) {
                for (int x = 0; x < carte.getLargeur(); x++) {
                    ...
                    writer.write(terrain.getAltitude() + " "); // Écrit l'altitude
                    writer.write(terrain.getHydrometrie() + " "); // Écrit l'hydrométrie
                    writer.write(terrain.getTemperature() + "\n"); // Écrit la température
                }
            }
        } catch (IOException | TerrainInexistant e) { // Gère les exceptions liées à l'entrée/sortie ou à l'absence de terrain
